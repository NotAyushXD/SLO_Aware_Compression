...
6. NO EXTERNAL DEPENDENCIES - runs standalone
...
# ============================================================================
# DIFFICULTY CONFIGURATION (Inline - no external dependency)
# ============================================================================

DIFFICULTY_MAX_TOKENS = {
    "easy": 100,
    "medium": 200,
    "hard": 300
}
...
DIFFICULTY_CONFIGS = {
    ...
    "medium": {
        "max_tokens": 200,
        "instruction": "Provide a clear answer with reasoning"
    },
    "hard": {
        "max_tokens": 300,
        "instruction": "Provide a detailed answer with full reasoning"
    }
}
    ...
    def evaluate(self) -> Tuple[Dict, List[PredictionResult]]:
        ...
        for i, example in enumerate(self.data_loader):
            try:
                ...
                # Use example's original prompt directly (no external dependency)
                full_prompt = example.get("prompt", "")
                
                # Get difficulty-aware token budget (inline)
                max_tokens = DIFFICULTY_MAX_TOKENS.get(difficulty, 200)
                ...
                # Use inline config (no external dependency)
                used_max_tokens = DIFFICULTY_MAX_TOKENS.get(difficulty, 200)
