...
- Two prompt modes:
    * slo      : shorter prompts / smaller token budgets
    * accuracy : stronger prompts / few-shot
...
def main(args: argparse.Namespace) -> None:
    ...
    logger.info(
        f"  Batching enabled: "
        f"{False if getattr(args, 'disable_batching', False) else True if getattr(args, 'enable_batching', False) else (args.prompt_mode == 'slo')}"
    )
    ...
    # ------------------------------------------------------------------
    # STEP 0 / 1: Preprocess or load data
    # ------------------------------------------------------------------
    if args.preprocess:
        ...
        train_data = load_jsonl(os.path.join(args.processed_dir, "train_data.jsonl"))
        val_data = load_jsonl(os.path.join(args.processed_dir, "val_data.jsonl"))
        test_data = load_jsonl(os.path.join(args.processed_dir, "test_data.jsonl"))

    logger.info(
        f"Loaded data: train={len(train_data)}, "
    ...
    server = SingleVariantServer(
        ...
        dtype=args.dtype,
        enable_batching=(
            False if getattr(args, "disable_batching", False)
            else True if getattr(args, "enable_batching", False)
            else (args.prompt_mode == "slo")
        ),
    ...
    if args.skip_load_test:
        ...
        for concurrency in args.concurrencies:
            ...
            if (
                ...
                and current_slos is None
                and concurrency == args.slo_calibrate_from_concurrency
            ):
                logger.info(
                    f"Calibrating SLOs from concurrency={concurrency} "
                    f"at p{args.slo_calibrate_percentile:.1f}"
                )
                current_slos = calibrate_slos(
                    request_metrics,
                    percentile=args.slo_calibrate_percentile,
                )
                try:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser("End-to-end baseline evaluation")

    p.add_argument("--preprocess", action="store_true")
    p.add_argument("--data_dir", type=str, default="data/raw")
    p.add_argument("--processed_dir", type=str, default="data/processed")
    p.add_argument("--data_subset", type=int, default=0)

    p.add_argument("--model_name", "--model", dest="model_name", type=str)
    ...
    p.add_argument("--prompt_mode", choices=["slo", "accuracy"], default="slo")
    ...
    p.add_argument("--slo_thresholds_path", type=str)
